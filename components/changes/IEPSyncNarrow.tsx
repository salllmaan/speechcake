...
>(({ className, children }, ref) => {
  return (
    <div
      ref={ref}
      className={cn(
        "z-10 flex size-12 items-center justify-center rounded-full border border-[#F0EFED] bg-white p-2",
        className,
      )}
    >
      {children}
...
export default function IEPSyncNarrow() {
  ...
  return (
    <div className="flex flex-col space-y-4">
      ...
      >
        <Circle ref={leftRef} className="h-12 w-auto rounded-full px-3 py-2">
          <span className="whitespace-nowrap text-[11px] font-semibold text-[#3A3A3A]">IEP System</span>
        </Circle>
        <Circle ref={rightRef} className="size-12 border-[#E8D6F0]">
          <Image
